...
void gameLoop(sf::RenderWindow& window,gameState& state ,textureLoader& textures, Player& player, Queue& enemies, Queue& flyingEnemies, sf::Sprite& floor1, sf::Sprite& floor2, float& randomSpawnGround, float& randomSpawnFlight, sf::Sprite& background,
               float& spawn_timer_ground, float& spawn_timer_flight, sf::Clock& pauseClock, float& animation_timer){
    sf::Event event;
    ...
    float frameDelta = fixed_dt;

    floor1.move(-obstacleSpeed * fixed_dt, 0);
    floor2.move(-obstacleSpeed * fixed_dt, 0);
    ...
    spawn_timer_ground += fixed_dt;
    bool spawned = enemySpawn(enemies, spawn_timer_ground, randomSpawnGround, textures);
    if(spawned){
        spawn_timer_ground = 0;
        randomSpawnGround = ((float)(rand() % (int)(maxSpawnTime * 100)) + minSpawnTime * 100) / 100;
    }
    spawn_timer_flight += fixed_dt;
    spawned = enemySpawn(flyingEnemies, spawn_timer_flight, randomSpawnFlight, textures);
    if(spawned){
        spawn_timer_flight = 0;
    ...
    animation_timer += fixed_dt;
    if(animation_timer > aFps){
    ...
    void restartGame(gameState& state, Player& player, textureLoader& textures, Queue& enemies, Queue& flyingEnemies, float& enemySpawnerGround, float& enemySpawnerFlight, sf::Clock& pauseClock, float& animation_timer, float& score){
        state = GAME;
        ...
        enemySpawnerGround = 0;
        enemySpawnerFlight = 0;
        pauseClock.restart();
        animation_timer = 0;
        score = 0;
