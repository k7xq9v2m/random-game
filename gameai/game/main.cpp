...
{   
    ...
    float spawn_timer_ground = 0;
    float spawn_timer_flight = 0;
    ...
    while (window.isOpen())
    {
        ...
        if(state == GAME) score += 10 * fixed_dt; 
        ...
        switch(state){
            case GAME:
                gameLoop(window, state, textures, player, enemies, flyingEnemies, floor1, floor2, randomSpawnGround, randomSpawnFlight, background,
                         spawn_timer_ground, spawn_timer_flight, pauseClock, animation_timer);
                         break;
            case MENU:
            ...
            case LOSESCREEN:
                ...
                if(restart) restartGame(state, player, textures, enemies, flyingEnemies, spawn_timer_ground, spawn_timer_flight, pauseClock,animation_timer, score);
                break;
        }
        window.display();
    } 
